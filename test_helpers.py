...
class EqsupplyTestCase(unittest.TestCase):
	...
	self.product_variant = ProductVariant.objects.create(product=self.product, part_number="S499989", weight="9.0", description="Car Wash Simulator", cost="122.34")

	# Quote
	self.quantity = 20
	self.quotation = Quotation.objects.create(user=self.user, time_created=datetime.now(), quotation_no="AGS 03-01-01", cost="120.00", status=False)
